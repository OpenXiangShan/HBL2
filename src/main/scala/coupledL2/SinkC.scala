...
class SinkC(implicit p: Parameters) extends L2Module {
  ...
  def toTaskBundle(c: TLBundleC): TaskBundle = {
    ...
    task.matrixTask.foreach(_ := isMatrix)
    task.ameChannel.foreach(_ := c.user.lift(AmeChannelKey).getOrElse(0.U))
    task.ameIndex.foreach(_ := c.user.lift(AmeIndexKey).getOrElse(0.U))
    task
  }
